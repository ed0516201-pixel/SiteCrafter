...
<body>
    <div class="container">
        ...
        <form method="POST" enctype="multipart/form-data" action="/upload">
            ...
            <input type="text" name="recipient" placeholder="@username or @botname" value="wsotp200bot" required>
        ...
        <div style="margin-top: 30px; padding-top: 20px; border-top: 2px solid rgba(255,255,255,0.2);">
            ...
            <div style="margin: 20px 0; padding: 15px; background: rgba(255,255,255,0.1); border-radius: 8px;">
                ...
                <p style="font-size: 13px; color: #ccc; margin: 10px 0;">Select specific groups to search for matching numbers. Leave empty to search all groups:</p>
                ...
                <div id="selectedGroupsDisplay" style="margin-top: 10px; padding: 10px; background: rgba(0,0,0,0.3); border-radius: 5px; min-height: 30px;">
                    <strong>Currently Selected:</strong> <span id="selectedGroupsText">No target groups selected (searching all groups)</span>
                </div>
            ...
            <div id="monitoringStatus" class="sending-status" style="display: block;">
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin: 10px 0;">
                    ...
                    <div class="progress-info" style="display: flex; justify-content: space-between; align-items: center;">
                        ...
                        <button type="button" id="resetDuplicatesBtn" onclick="resetDuplicates()" 
                                style="padding: 4px 12px; font-size: 12px; background: #e74c3c; color: white; border: none; border-radius: 4px; cursor: pointer;">
                            Clear
                        </button>
                    </div>
                    <div class="progress-info">
    ...
    <script>
        ...
        function checkSendingStatus() {
            ...
            .then(data => {
                ...
                if (data.error_message) {
                    document.getElementById('sendingStatus').style.display = 'block';
                    document.getElementById('statusText').textContent = '❌ Error: ' + data.error_message;
                    document.getElementById('statusText').style.color = '#e74c3c';
                    document.getElementById('sendButton').disabled = false;
                    document.getElementById('sendButton').textContent = 'Send to Recipient';
                    if (sendingInterval) {
                ...
                if (data.is_sending) {
                    ...
                    // Update progress info and percentage
                    const percentage = data.total_messages > 0 ? Math.round((data.current_message / data.total_messages) * 100) : 0;
                    ...
                    const successCount = data.messages_sent_successfully || 0;
                    document.getElementById('successCount').textContent = successCount;
                    document.getElementById('failedCount').textContent = data.messages_failed || 0;
                    document.getElementById('sendingSpeed').textContent = data.sending_speed || 0;
                    
                    // Update prominent total messages sent display
                    document.getElementById('totalMessagesSent').textContent = successCount;
                    
                    // Update time remaining with proper integer handling
                    const timeRemaining = Math.floor(data.estimated_time_remaining || 0);
                    const minutes = Math.floor(timeRemaining / 60);
                    const seconds = timeRemaining % 60;
                    document.getElementById('timeRemaining').textContent = timeRemaining > 0 ? `${minutes}:${seconds.toString().padStart(2, '0')}` : '--:--';
                    
                    // Update detailed info with safe defaults
                    document.getElementById('currentRecipient').textContent = data.current_recipient || '--';
                    const sendModeText = data.send_mode === 'columns' ? 'Column by Column' : 
                                       data.send_mode === 'rows' ? 'Row by Row' : 
                                       data.send_mode || 'Unknown';
                    document.getElementById('sendMode').textContent = sendModeText;
                    document.getElementById('currentNumber').textContent = data.current_number || '--';
                    const lastMsg = String(data.last_message_sent || '--');
                    document.getElementById('lastMessageSent').textContent = lastMsg.substring(0, 50) + (lastMsg.length > 50 ? '...' : '');
                    
                    // Handle pause countdown display
                    if (data.is_paused && data.pause_countdown > 0) {
                        const countdownMinutes = Math.floor(data.pause_countdown / 60);
                        const countdownSeconds = data.pause_countdown % 60;
                        const countdownText = `${countdownMinutes}:${countdownSeconds.toString().padStart(2, '0')}`;
                        document.getElementById('statusText').textContent = `⏸️ Paused - Next batch in: ${countdownText}`;
                        document.getElementById('statusText').style.color = '#f39c12'; // Orange color for pause
                        document.getElementById('currentNumber').textContent = `Waiting for 2-minute pause to complete...`;
                    } else if (data.is_paused) {
                        document.getElementById('statusText').textContent = '⏸️ Paused - Resuming shortly...';
                        document.getElementById('statusText').style.color = '#f39c12'; // Orange color for pause
                    } else {
                        document.getElementById('statusText').textContent = '📤 Sending Messages...';
                        document.getElementById('statusText').style.color = '#4a90e2'; // Default blue color
                    }
                    ...
                    // Not sending - hide stop button and reset UI
                    if (sendingInterval) {
                        clearInterval(sendingInterval);
                        sendingInterval = null;
                    }
                    document.getElementById('sendingStatus').style.display = 'none';
                    document.getElementById('sendButton').disabled = false;
                    document.getElementById('sendButton').textContent = 'Send to Recipient'; // Reset button text
                    document.getElementById('stopButton').disabled = false;
                    document.getElementById('stopButton').textContent = '🛑 Stop Sending';
                    document.getElementById('stopButton').classList.add('pulse');
                    document.getElementById('statusText').textContent = '📤 Sending Messages...';
                }
            })
            .catch(error => console.error('Error:', error));
        ...
        // Handle target recipient form
        document.getElementById('setTargetButton').addEventListener('click', function() {
            const recipient = document.getElementById('targetRecipientInput').value.trim();
            const statusDiv = document.getElementById('targetStatus');
            
            if (!recipient) {
                statusDiv.innerHTML = '<span style="color: #e74c3c;">Please enter a target recipient</span>';
                return;
            }
            ...
            statusDiv.innerHTML = '<span style="color: #f39c12;">Setting target recipient...</span>';
            
            // Send request to set target recipient
            const formData = new FormData();
            formData.append('recipient', recipient);
            
            fetch('/set_target_recipient', {
                method: 'POST',
                body: formData
            })
            .then(response => response.json())
            .then(data => {
                if (data.status === 'success') {
                    statusDiv.innerHTML = `<span style="color: #27ae60;">✅ Target set: ${data.message}</span>`;
                    // Update the monitoring status immediately
            ...
            .catch(error => {
                console.error('Error setting target recipient:', error);
                statusDiv.innerHTML = '<span style="color: #e74c3c;">❌ Failed to set target recipient</span>';
            })
            .finally(() => {
                // Re-enable button
                this.disabled = false;
                this.textContent = 'Set Target';
            });
        });
        
        // Target Groups Management JavaScript
        let availableGroups = [];
        let selectedGroupIds = [];
        
        document.getElementById('loadGroupsButton').addEventListener('click', function() {
            ...
            statusDiv.innerHTML = '<span style="color: #f39c12;">Loading available groups...</span>';
            
            fetch('/get_available_groups')
            .then(response => response.json())
            .then(data => {
                if (data.status === 'success') {
                    availableGroups = data.groups;
                    selectedGroupIds = data.current_target_groups || [];
                    
                    // Display groups with checkboxes
                    if (availableGroups.length === 0) {
                        groupsList.innerHTML = '<p style="text-align: center; color: #ccc;">No groups found</p>';
                    } else {
                        let groupsHtml = '<div style="max-height: 150px; overflow-y: auto;">';
                        availableGroups.forEach(group => {
                            const isChecked = selectedGroupIds.includes(group.id) ? 'checked' : '';
                            const groupType = group.type === 'channel' ? '📢' : '👥';
                            groupsHtml += `
                                <div style="margin: 5px 0; padding: 5px; border: 1px solid rgba(255,255,255,0.1); border-radius: 3px;">
                                    <label style="display: flex; align-items: center; cursor: pointer;">
                                        <input type="checkbox" value="${group.id}" ${isChecked} style="margin-right: 8px;" onchange="updateGroupSelection()">
                                        ${groupType} <strong>${group.name}</strong> <span style="color: #888; font-size: 12px;">(ID: ${group.id})</span>
                                    </label>
                                </div>
                            `;
                        });
                    ...
                    statusDiv.innerHTML = `<span style="color: #27ae60;">✅ Loaded ${availableGroups.length} groups</span>`;
            ...
            .catch(error => {
                ...
                statusDiv.innerHTML = '<span style="color: #e74c3c;">❌ Failed to load groups</span>';
            })
            .finally(() => {
                this.disabled = false;
                this.textContent = 'Load Groups';
            });
        });
        ...
        function updateSelectedGroupsDisplay() {
            ...
            if (selectedGroupIds.length === 0) {
                selectedGroupsText.textContent = 'No target groups selected (searching all groups)';
                selectedGroupsText.style.color = '#ccc';
            } else {
                const selectedNames = selectedGroupIds.map(id => {
                    const group = availableGroups.find(g => g.id === id);
                    return group ? group.name : `ID: ${id}`;
                }).join(', ');
                selectedGroupsText.textContent = `${selectedGroupIds.length} group(s): ${selectedNames}`;
                selectedGroupsText.style.color = '#27ae60';
            }
        }
        
        document.getElementById('clearGroupsButton').addEventListener('click', function() {
            selectedGroupIds = [];
            const checkboxes = document.querySelectorAll('#groupsList input[type="checkbox"]');
            checkboxes.forEach(cb => cb.checked = false);
            updateSelectedGroupsDisplay();
            document.getElementById('groupsStatus').innerHTML = '<span style="color: #f39c12;">Selection cleared</span>';
        });
        
        document.getElementById('saveTargetGroupsButton').addEventListener('click', function() {
            const statusDiv = document.getElementById('groupsStatus');
            
            // Disable button while saving
            this.disabled = true;
            this.textContent = 'Saving...';
            statusDiv.innerHTML = '<span style="color: #f39c12;">Saving target groups...</span>';
            
            // Send request to save target groups
            fetch('/set_target_groups', {
                method: 'POST',
                headers: {
            ...
            .catch(error => {
                console.error('Error saving target groups:', error);
                statusDiv.innerHTML = '<span style="color: #e74c3c;">❌ Failed to save target groups</span>';
            })
            .finally(() => {
                this.disabled = false;
                this.textContent = 'Save Target Groups';
            });
        });
        ...
        function checkMonitoringStatus() {
            ...
            .then(data => {
                ...
                if (!data.monitoring) {
                    document.getElementById('monitoringState').textContent = 'Monitoring offline';
                    document.getElementById('monitoringState').style.color = '#e74c3c';
                } else if (data.has_target_set) {
                    document.getElementById('monitoringState').textContent = `Monitoring ${data.target_recipient}`;
                    document.getElementById('monitoringState').style.color = '#27ae60';
                } else {
                    document.getElementById('monitoringState').textContent = 'Active - No target set';
                    document.getElementById('monitoringState').style.color = '#f39c12';
                }
                
                // Update counts (target-scoped)
                ...
                document.getElementById('matchesFound').textContent = data.matches_found || 0;
                document.getElementById('groupNumbersCount').textContent = data.group_numbers_count || 0;
            })
            .catch(error => {
                console.error('Error fetching monitoring status:', error);
                document.getElementById('monitoringState').textContent = 'Error loading status';
                document.getElementById('monitoringState').style.color = '#e74c3c';
            });
        }
        
        // Check monitoring status on page load and periodically
        checkMonitoringStatus();
        setInterval(checkMonitoringStatus, 3000); // Update every 3 seconds
        ...
        function checkSendingStatusOnce() {
            ...
            .then(data => {
                if (data.is_sending) {
                    ...
                    // Only start interval if not already running with 0.5 second real-time updates
                    if (!sendingInterval) {
                        sendingInterval = setInterval(checkSendingStatus, 500);
                    }
                }
        ...
        // Start monitoring for reply status and message sending when page loads
        window.addEventListener('load', function() {
            checkSendingStatusOnce();
            checkMonitoringStatusOnce();
            // Monitoring interval is started by checkMonitoringStatusOnce if needed
        });
        
        // Monitoring is always active - no manual button controls needed
        
        function checkMonitoringStatus() {
            fetch('/monitoring_status')
            .then(response => response.json())
            .then(data => {
                document.getElementById('monitoringState').textContent = data.monitoring ? 'Active' : 'Stopped';
                ...
                document.getElementById('groupNumbersCount').textContent = data.group_numbers_count;
                
                // Always active monitoring - no button controls needed
                
                if (data.monitoring) {
                    if (!monitoringInterval) {
                        monitoringInterval = setInterval(checkMonitoringStatus, 3000);
                    }
                } else {
                    if (monitoringInterval) {
        ...
        function checkMonitoringStatusOnce() {
            fetch('/monitoring_status')
            .then(response => response.json())
            .then(data => {
                document.getElementById('monitoringState').textContent = data.monitoring ? 'Active' : 'Stopped';
                ...
                if (data.monitoring && !monitoringInterval) {
                    monitoringInterval = setInterval(checkMonitoringStatus, 3000);
                }
            })
            .catch(error => console.error('Error:', error));
        ...
        function resetDuplicates() {
            if (!confirm('Are you sure you want to clear all duplicate counts? This will reset the duplicate counter to 0.')) {
                return;
            }
            
            fetch('/reset_duplicates', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                }
            ...
            .then(data => {
                if (data.status === 'success') {
                    alert('Duplicate counts have been reset to 0');
                    checkMonitoringStatus();
                } else {
            ...
            .catch(error => {
                ...
                alert('Failed to reset duplicates');
            });
        }
    </script>
</body>
